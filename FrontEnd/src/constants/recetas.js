export const TOP_RECETAS = [
  {
    titulo: "Ensalada",
    autor: "Francisco",
    foto: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
  },
  {
    titulo: "Ensalada",
    autor: "Francisco",
    foto: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
  },
  {
    titulo: "Ensalada",
    autor: "Francisco",
    foto: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
  },
  {
    titulo: "Ensalada",
    autor: "Francisco",
    foto: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
  },
  {
    titulo: "Ensalada",
    autor: "Francisco",
    foto: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
  },
];

export const RECETAS = [
  {
    id: 1,
    titulo: "Ensalada",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 1, nombre: "tomate", cantidad: "2" },
      { id: 2, nombre: "lechuga criolla", cantidad: "1" },
      { id: 3, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 1,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 2,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 3,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 15,
  },
  {
    id: 2,
    titulo: "Ensalada 2",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 1, nombre: "tomate", cantidad: "2" },
      { id: 2, nombre: "lechuga criolla", cantidad: "1" },
      { id: 3, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 1,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 2,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 3,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 15,
  },
  {
    id: 3,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 4,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 5,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 6,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 7,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 8,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 10,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 11,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 12,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 13,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 14,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 15,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 16,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 17,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 18,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 19,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
  {
    id: 20,
    titulo: "Ensalada 3",
    descripcion: "Ensalada se tomate, lechuga y cebolla",
    fotos: [
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSN1jS0csAnOZ7P6srzfhfyPBXePsMXsElAMXPtPMjDhALjRaH48k6CP91kfK2rb8Pi790&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ_E9aLQsZQOdcd3oxF2aTcoviX_R9hn8-FKJUEkEvkAhBSkI4JUxB_tBzYAs04B05esVE&usqp=CAU",
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTul1c2ONOF9ZmtZ9e7xTF6cxcsQW6n5WQ6a8VwFKqx8nOuIwt94_MY938xHQW4NOiXJFU&usqp=CAU",
    ],
    ingredientes: [
      { id: 4, nombre: "tomate", cantidad: "2" },
      { id: 5, nombre: "lechuga criolla", cantidad: "1" },
      { id: 6, nombre: "cebolla", cantidad: "1/2" },
    ],
    categoria: "Acompañamiento",
    pasos: [
      {
        id: 4,
        numero: 1,
        descripcion:
          "Lavar el tomate y las hojas de lechuga, ademas pelar la cebolla",
      },
      {
        id: 5,
        numero: 2,
        descripcion:
          "cortar el tomate en cubos, la lechuga en tiras y la cebolla en corte juliana",
      },
      {
        id: 6,
        numero: 3,
        descripcion:
          "condimentar la ensalada con sal, aceite, vinagre y servir",
      },
    ],
    tiempoAprox: 25,
  },
];
